...
void PointCloudData::InitPointCloud(PCPoint* pointCloud)
{
	...
	for (int num = 0; num < m_PointSumNumber; num++)
	{
		...
		{
			v[j] = pointCloud[num].m_Coordinate[j];
	...
	//PointMoveToOrigin(pointCloud);//将模型置于显示中心  zcl
	for (int num = 0; num < m_PointSumNumber; num++)
	{
		for (int j = 0; j < 3; j++)
		{
...
bool PointCloudData::BoundaryCheck(PCPoint lpoint)
{
	...
	if (abs(((double)side1 - (double)side2) / BCNum) > RThreadshold)
	{
		return true;
	}
	return false;
...
//进行边界聚类
void PointCloudData::BoundaryClustering(pcl::PointCloud<pcl::PointXYZ>::Ptr boundarycloud)
{
	//获取选中的边界点集合
	...
	ec.setClusterTolerance(m_clusterDistance);
	...
	for (int num = 0; num < m_CloudClusterNum; num++)
	{
		...
		for (int i = 0; i < m_BoundaryPointClusters[num].size(); i++)
		{
			...
			for (int j = 1; j < knum; j++)
			{
				...
				{
					selectedPoint.push_back(RkNearestPoints[j]);//zcl边界点周围的knum个点
				}
			}
			RkNearestPoints.clear();
		}
...
void PointCloudData::SurfaceGenerating()
{
	...
	double m_trianMaxDistace = 200.0f;//最大连接距离
	double m_trianMaxAngle = 2.5f;//最大角
	double m_trianMinAngle = 18;//最小角
	double m_trianMaxSAngle = 3;//最远搜索距离
	...
	gp3.setMaximumAngle(2 * M_PI / m_trianMaxSAngle);//设置三角化后得到三角形内角最大角度
	...
	gp3.reconstruct(triangles);//可以补充基于三角网格的点云修补
	...
	*/
	...
	//	vtkSmartPointer<vtkTriangle> sanjiao = vtkSmartPointer<vtkTriangle>::New();
	//	int id1, id2, id3;
	...
	//std::cout << "拓扑结构数量"<< polydata->GetNumberOfCells() << std::endl;
	//std::cout << "几何结构数量" << polydata->GetNumberOfPoints() << std::endl;
	...
	//fillHolesFilter->SetHoleSize(5000);//指定要填充的孔的最大尺寸（半径）
...
void PointCloudData::PCLBoundaryKeep()
{
	...
	n.setRadiusSearch(RThreadshold);
	pcl::PointCloud<pcl::Normal>::Ptr normals(new pcl::PointCloud<pcl::Normal>);
	n.compute(*normals);
	...
	boundEst.setRadiusSearch(RThreadshold);
	boundEst.setAngleThreshold(BAngleThreshold);//边界判断时的角度阈值
